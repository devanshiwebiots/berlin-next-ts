...
const CartAction = () => {
  ...
  return (
    <>
      ...
      <tr>
        <td className="text-end" colSpan={5}><Link href={`/app/ecommerce/product`} className="btn btn-secondary cart-btn-transform">{ContinueShopping}</Link></td>
        <td><Link className="btn btn-success cart-btn-transform" href={`/app/ecommerce/checkout`}>{CheckOut}</Link></td>
      </tr>
    </>
  );
};
